...
记录如下 ： （本篇为基本配置 ， 不含 build 等生命周期相关）

# 基本知识

1、maven 管理 jar 包 ， 定位唯一 jar 包会通过 “ gav ” 坐标。（groupId 、 artifactId、version）先在项目路径下找 jar 包， 找不到的话再去 mvn 仓库中搜索。

2、maven 的三种关系 ：继承、 聚合、 依赖。这种在多模块项目中是常见的。

继承 ： 通过 parent 标签实现， 子工程继承父工程全部依赖。

聚合 ： 通过 module 标签实现， 所谓的多模块项目，子模块聚合于父模块内部。

依赖 ： 常规 dependency ，引用 jar 包。

常见多模块项目，子模块与父模块常常是 继承 + 聚合。

# idea 配置

1、 配置路径 ： `File - Settings - Build - Build Tools - Maven`

2、 默认 maven 没有 settings file ， 勾选 override ， 在对应的路径下创建 settings.xml ，参考百度， 替换阿里镜像数据源。
注 ： 如果指定了 local repository 位置，默认配置可能不生效。

3、local repository 是 maven 管理 jar 包位置。 在遇到已存在 jar 包工程中却还是找不到的bug时，尝试手动删除仓库中 jar 包 ， 重新导入 pom 文件。

4、`File - Settings - Build - Build Tools - Maven - importing` ： 将 jdk 勾选为相应的 jre
